    ...
    useEffect(() => {
        ...
        if (foundRestaurant) {
            if (isMounted) {
                ...
                setLoading(false);
                actions.obtenerUnaCategoria(foundRestaurant.categorias_id).then((categoryData) => {
                    if (isMounted) setCategoryName(categoryData ? categoryData.nombre_de_categoria : "No especificado");
                });
            }
        } else {
