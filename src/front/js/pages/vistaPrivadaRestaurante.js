...
export const VistaPrivadaRestaurante = () => {
  ...
  const [showPasswordModal, setShowPasswordModal] = useState(false); // Estado para mostrar el modal
  ...
  const handleUpdateRestaurante = async () => {
    try {
      // Llamada a la API para modificar los datos del restaurante
      const response = await actions.modificarDatosRestaurante(restaurante_id, formData);

      if (response.success) {
        // 1. Actualiza el estado local del restaurante con los nuevos datos
        setFormData(response.data);

        //  Actualiza el store global si es necesario
        ...
        // 3. Notificar al usuario que los datos fueron actualizados
  ...
  return (
    <div className="restaurant-private-container">
      ...
      <p className="restaurant-private-info"><strong>Dirección:</strong> {restaurante.direccion}</p>
      <p className="restaurant-private-info"><strong>Nombre del restaurante:</strong> {restaurante.nombre}</p>
      <p className="restaurant-private-info"><strong>Teléfono:</strong> {restaurante.telefono}</p>
      <p className="restaurant-private-info"><strong>Cubiertos:</strong> {restaurante.cubiertos}</p>
      <p className="restaurant-private-info"><strong>Cantidad de mesas:</strong> {restaurante.cantidad_mesas}</p>
      <p className="restaurant-private-info"><strong>Reservas por día:</strong> {restaurante.reservas_por_dia}</p>

      <p className="restaurant-private-info">
        <strong>Horario de mañana:</strong> {restaurante.horario_mañana_inicio} - {restaurante.horario_mañana_fin}
      </p>
      <p className="restaurant-private-info">
        <strong>Horario de tarde:</strong> {restaurante.horario_tarde_inicio} - {restaurante.horario_tarde_fin}
      </p>

      <p className="restaurant-private-info">
        <strong>Favoritos:</strong> {restaurante.favoritos_count || 0} usuarios han añadido este restaurante a sus favoritos.
      </p>


      {/* Botón para cambiar la contraseña */}
      {/* Botón para cambiar la contraseña */}
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#recuperacion">
        Cambiar contraseña
      </button>
      ...
      {restaurante.image && (
        <img className="restaurant-private-image" src={restaurante.image} alt={`Imagen de ${restaurante.nombre}`} />
      )}
      <div className="restaurant-private-form">
        ...
        {/* Cubiertos */}
        <div className="mb-3">
          <label className="form-label" htmlFor="cubiertos">Cubiertos</label>
          <input
            className="form-control"
            type="text"
            name="cubiertos"
        ...
        {/* Cantidad de mesas */}
        <div className="mb-3">
          <label className="form-label" htmlFor="cantidad_mesas">Cantidad de mesas</label>
          <input
            className="form-control"
            type="text"
            name="cantidad_mesas"
        ...
        {/* Reservas por día */}
        <div className="mb-3">
          <label className="form-label" htmlFor="reservas_por_dia">Reservas por día</label>
          <input
            className="form-control"
            type="text"
